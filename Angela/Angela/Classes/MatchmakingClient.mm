...
-(void)searchAvailServers{
    NSArray* servers = [session peersWithConnectionState:GKPeerStateAvailable];
    for(NSString* peerID in servers){
        if (![self.availableServers containsObject:peerID]) {
            [self.availableServers addObject:peerID];
            _delegate->serverBecameAvailable(self, peerID);
        }
...
- (NSInteger)getAvailServersIndex:(NSString*) peerID{
    for(int i=0;i< [self.availableServers count];i++)
        if(self.availableServers[i]==peerID)
            return i;
    return -1;
}
